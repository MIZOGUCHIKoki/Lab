...
\begin{frame}{}
    ...
    \begin{figure}[h]
        ...
        \begin{tikzpicture}[remember picture]
            ...
            \onslide<2->{
            \shade[shading=ball, ball color=blue] (A) circle [radius=0.2cm]node[white]{1};
            }
            \onslide<3->{
            \shade[shading=ball, ball color=blue] ($(B.east)!0.5!(ead.west)$) circle [radius=0.2cm]node[white]{2};
            }
        \end{tikzpicture}
    \end{figure}
    \begin{enumerate}
        \item<2-> 認証情報に一方向ハッシュ関数を施す．
        \item<3-> \tikz[baseline=(cp1.base)]{\node[draw](cp1){比較}}で{\ttfamily Client}が{\ttfamily Server}を検証する．
            \begin{itemize}
                \item<4-> 不一致の場合，認証不成立．（{\ttfamily Server}が不正である可能性あり）
            \end{itemize}
    \end{enumerate}
\end{frame}
\begin{frame}{}
    ...
    \onslide<2->{
        \begin{equation}
            ...
            \end{aligned}\tag*{(\tikz[baseline=(w3.base)]{\shade[shading=ball, ball color=blue] (A) circle [radius=0.2cm]node(w3)[white]{3}})}
        \end{equation}
    ...
    \begin{enumerate}
        \setcounter{enumi}{2}
        \item<2-> 次回認証情報，\tikz[baseline=(nad_s.base)]{\node[fill=blue!10](nad_s){\(A_{i+1}\oplus A_{i+2}\)}}を{\ttfamily Client}に保存．
        \item<3-> \(\gamma_i=E_2\Big(A_{i+1}\oplus A_{i+2}\Big)\)を生成．
        \item<4-> \(\gamma_i\)を{\ttfamily Server}へ送信．
    \end{enumerate}
\end{frame}
\begin{frame}{}
    \begin{center}
    ...
    \begin{enumerate}
        \setcounter{enumi}{5}
        \item<2-> 生成済みデータから，\tikz[remember picture,baseline=(md.base)]{\node[fill=orange!10](md){\(E_2\Big(A_{i+1}\oplus A_{i+2}\Big)\)}}を生成する．
        \item<3-> 受信データ\(\gamma_i\)と比較する．
            \begin{itemize}
                \item<4-> 不一致の場合，認証不成立．（{\ttfamily Client}が不正である可能性あり．）
                \item<4-> 一致した場合，認証成立．
            \end{itemize}
        \item<5-> \(\gamma_i\)を利用した共通鍵暗号方式で通信を行う．
    \end{enumerate}
\end{frame}
\begin{frame}[c]{}
    \begin{block}{軽量度}
